...
이 폴더에는 최초 공개 릴리스의 기반이 된 `schemaVersion: 1` 정본 데이터와 원곡 기준 정보를 둡니다. 현재 사이트가 직접 읽는 공개 응원가·구단 데이터는 `src/data/generated/catalog.json`이며, Admin 공개 동작이 `content/releases/`의 불변 스냅샷에서 이를 생성합니다. 최초 정본에는 사용자가 검수한 응원가 23곡이 포함되어 있습니다.
