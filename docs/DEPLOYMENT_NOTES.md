...
- 상태 확인: `/healthz`
- 영속 데이터와 비밀값: 없음
- 공개 환경 설정: `GA_MEASUREMENT_ID=G-8TSWVPVC8T` (GA4 웹 데이터 스트림 측정 ID)
...
운영 배포 후 해당 페이지를 직접 열고 GA4 **보고서 → 실시간**에서 방문이 들어오는지 확인한다. 유입은 즉시 보이지 않을 수 있다. 코드가 `G-...` ID를 사용하므로 별도의 Google Tag Manager 태그를 같은 페이지에 중복 설치하지 않는다.
