...
`원곡 순서` 화면에서는 메인페이지 `원곡별` 목록의 순서를 드래그, 화살표 또는 위치 번호로 조정하고 저장할 수 있습니다. 각 원곡에 등록된 응원가를 대표·보조 연결과 공개 상태별로 확인하고 편집 화면을 열 수 있습니다. 저장된 순서는 `data/original-songs.json`에 기록되며, 운영 사이트에는 다음 빌드·배포 때 반영됩니다.
...
Admin에서 AI 수집·작성을 요청하면 Naru/Codex가 같은 저장소에서 현재 본문을 포함한 작업을 가져옵니다. Codex는 흥미로운 소재 약 10개를 목표로 조사해 소재·확신도·출처를 적은 **소재 노트**만 JSON으로 돌려주며, 공개 문장은 쓰지 않습니다. 공개 본문은 `작업 모드 → 다듬기`에서 여러 곡의 소재 노트를 ChatGPT 요청문 하나로 복사하고, 답변을 붙여 넣어 곡별로 반영합니다.

`작업 모드`에서는 영상 고르기·가사·다듬기·공개 준비를 작업 종류별로 이어서 처리합니다. 영상 고르기는 `yt-dlp`로 곡마다 후보 10개를 모으고, 댓글·설명란·챕터의 타임스탬프로 곡이 시작되는 초를 제안합니다. 후보를 임베드로 재생하며 시작 초를 조정하고 대표·추가 영상을 고르면 `t=` 시작 초가 포함된 URL로 저장되어 사이트에서도 그 지점부터 재생됩니다. 영상과 가사가 채워진 곡은 본문을 다듬기 전에도 공개할 수 있으며, 초안 본문은 사이트에 표시되지 않습니다.

```bash
node scripts/video-candidates-cli.mjs collect --missing
```
